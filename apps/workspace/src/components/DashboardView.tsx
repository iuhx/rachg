...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div className="workspace-view space-y-8 pb-20">
      ...
      <section className="workspace-card p-4 sm:p-5" aria-labelledby="scratchpad-heading"><div className="flex items-center justify-between mb-3"><div className="type-section-heading flex items-center gap-2 text-neutral-900 dark:text-white"><Paperclip className="w-4 h-4 text-neutral-400" /><span id="scratchpad-heading">Scratchpad</span></div><span className="type-caption">Temporary space</span></div><textarea value={scratchContent} onChange={(event) => setScratchContent(event.target.value)} onPaste={handlePaste} onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop} placeholder="Paste a thought, link, or image…" className={`workspace-input workspace-editor min-h-[5.5rem] ${isDragging ? 'border-neutral-500 bg-neutral-100 dark:bg-white/10' : ''}`} />{pendingImage && <div className="workspace-panel type-secondary mt-2 px-3 py-2 flex items-center justify-between"><span>Image ready: {pendingImage.name}</span><button onClick={() => setPendingImage(null)} className="type-label text-red-500 cursor-pointer">Remove</button></div>}{scratchpad?.imageUrl && <img src={scratchpad.imageUrl} alt="Scratchpad attachment" className="max-h-36 max-w-full rounded-lg object-contain mt-3" />}{!isScratchpadLoading && <div className="flex justify-end gap-2 mt-2"><button onClick={() => void onClearScratchpad()} disabled={isScratchpadSaving} className="workspace-button workspace-button-secondary cursor-pointer">Clear</button><button onClick={() => void saveScratch()} disabled={isScratchpadSaving} className="workspace-button workspace-button-primary cursor-pointer">{isScratchpadSaving ? 'Saving…' : 'Save'}</button></div>}{isScratchpadLoading && <p className="type-caption mt-2">Loading scratchpad…</p>}</section>
