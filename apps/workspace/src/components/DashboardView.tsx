...
import { Activity, ArrowUpRight, Check, ChevronRight, Copy, FileText, FolderDot, Image as ImageIcon, Loader2, Paperclip, Plus, Upload } from 'lucide-react';
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const activities = [
    ...notes.slice(0, 3).map((note) => ({ id: `n-${note.id}`, label: 'Note', title: note.title, detail: note.updatedAt, tab: 'notes' as NavTab })),
    ...transfers.slice(0, 3).map((file) => ({ id: `f-${file.id}`, label: 'File', title: file.name, detail: file.updatedAt, tab: 'files' as NavTab })),
    ...projects.slice(0, 2).map((project) => ({ id: `p-${project.id}`, label: 'Project', title: project.name, detail: project.updatedAt, tab: 'projects' as NavTab })),
  ].slice(0, 6);
  ...
  return (
    <div className="workspace-view space-y-8 pb-20">
      ...
      <section className="workspace-card p-5 sm:p-6" aria-labelledby="scratchpad-heading"><div className="flex items-center justify-between mb-4"><div className="type-section-heading flex items-center gap-2 text-neutral-900 dark:text-white"><Paperclip className="w-4 h-4 text-neutral-400" /><span id="scratchpad-heading">Scratchpad</span></div><span className="type-caption">Temporary space</span></div><textarea value={scratchContent} onChange={(event) => setScratchContent(event.target.value)} onPaste={handlePaste} onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop} placeholder="Paste a thought, link, or image…" className={`workspace-input workspace-editor min-h-[9rem] ${isDragging ? 'border-neutral-500 bg-neutral-100 dark:bg-white/10' : ''}`} />{pendingImage && <div className="workspace-panel type-secondary mt-2 px-3 py-2 flex items-center justify-between"><span>Image ready: {pendingImage.name}</span><button onClick={() => setPendingImage(null)} className="type-label text-red-500 cursor-pointer">Remove</button></div>}{scratchpad?.imageUrl && <img src={scratchpad.imageUrl} alt="Scratchpad attachment" className="max-h-48 max-w-full rounded-lg object-contain mt-3" />}{!isScratchpadLoading && <div className="flex justify-end gap-2 mt-3"><button onClick={() => void onClearScratchpad()} disabled={isScratchpadSaving} className="workspace-button workspace-button-secondary cursor-pointer">Clear</button><button onClick={() => void saveScratch()} disabled={isScratchpadSaving} className="workspace-button workspace-button-primary cursor-pointer">{isScratchpadSaving ? 'Saving…' : 'Save'}</button></div>}{isScratchpadLoading && <p className="type-caption mt-2">Loading scratchpad…</p>}</section>
      {activities.length > 0 && <section className="workspace-card p-5 sm:p-6" aria-labelledby="recent-activity-heading"><div className="flex items-center justify-between mb-4"><div className="type-section-heading flex items-center gap-2 text-neutral-900 dark:text-white"><Activity className="w-4 h-4 text-neutral-400" /><span id="recent-activity-heading">Recent Activity</span></div><span className="type-caption">Your workspace</span></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1">{activities.map((activity) => <button key={activity.id} onClick={() => onNavigate(activity.tab)} className="text-left p-3 rounded-lg hover:bg-neutral-50 dark:hover:bg-white/[0.03] cursor-pointer"><span className="type-caption block">{activity.label}</span><span className="type-body block text-neutral-800 dark:text-neutral-200 truncate mt-1">{activity.title}</span><span className="type-caption block mt-1">{activity.detail}</span></button>)}</div></section>}
